...
NextFit() {
    ...
    for ((i = 0; i < n; i++)); do
        local found=0  # Flag to indicate if a block is found for the process
        ...
        while ((j < m)); do
            if ((${blockSize[$j]} >= ${processSize[$i]})); then
                ...
                t=$(( (j - 1) % m ))
                found=1  # Mark that a block has been found
                break
            fi
            if ((t == j)); then
                # sets a new end point
        ...
        # If no suitable block is found for the process, mark it as "Not Allocated"
        if ((found == 0)); then
            allocation[$i]=-1
        fi
    done

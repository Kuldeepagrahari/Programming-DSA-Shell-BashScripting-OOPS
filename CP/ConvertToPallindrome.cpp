...
// -------------------------------------------divide & conquer---------------
double DBLpower(double x, long n)
{
    // int ans=x;
    if (n == 0)
    {
...
for(auto&sam:mp){
    even="";
  if(sam.second%2==0){
    fl(i,0,sam.second)even+=sam.first;
    ans.insert(i,even);i+=sam.second/2;
 
    }
    else {
        if(flag==0){
        flag=1;
        fl(i,0,sam.second)odd+=sam.first;
...
if(l&1)if(ans!="NO SOLUTION") ans.insert(i,odd);
cout<<ans<<endl;
